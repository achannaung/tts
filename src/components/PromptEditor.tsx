...
export const PromptEditor: React.FC<PromptEditorProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#f8f9fa] overflow-hidden">
      ...
      <div className="h-12 bg-white border-b border-gray-200 px-6 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center space-x-2 text-xs text-slate-700 font-medium">
            ...
            <div className="relative inline-block">
              <select
                ...
                className="bg-gray-100 hover:bg-gray-200/80 border border-gray-300 rounded-md px-2.5 py-1 pr-6 text-xs font-semibold text-slate-800 outline-none cursor-pointer appearance-none transition-colors"
              >
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            disabled={!text.trim()}
            className="px-3 py-1 text-xs font-semibold bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-md transition-all flex items-center space-x-1.5 disabled:opacity-40 shadow-xs"
            title="Open Gemini 3.6 Flash Script Polish Studio"
          >
            <Wand2 className="w-3.5 h-3.5 text-blue-200" />
            <span>AI Polish Studio</span>
          </button>
      ...
      {showSystemPrompt && (
        <div className="bg-slate-50 border-b border-gray-200 p-4 space-y-1.5 animate-fadeIn">
          <div className="flex items-center justify-between text-[11px]">
            ...
            <span className="text-slate-400">Injected into TTS generation context</span>
          </div>
          <input
            type="text"
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-2 flex items-center space-x-2 overflow-x-auto custom-scrollbar shrink-0">
      ...
      <div className="flex-1 p-6 relative flex flex-col overflow-hidden">
        <textarea
          ...
          className="w-full flex-1 p-4 border border-gray-200 rounded-xl bg-white text-slate-800 placeholder-slate-400 text-lg leading-relaxed focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none resize-none shadow-xs"
        />
        ...
        {onSynthesize && (
          <div className="absolute bottom-10 right-10 z-10">
            <button
              ...
              className={`px-6 py-3 rounded-xl font-bold text-sm text-white shadow-lg transition-all flex items-center space-x-2.5 active:scale-95 ${
            ...
            >
              {isGenerating ? (
                ...
                <>
                  ...
                  <span className="text-[10px] bg-blue-700/80 px-2 py-0.5 rounded text-blue-100 font-mono">
                    ⌘↵
                  </span>
                </>
              )}
      ...
      {/* Editor Footer Stats */}
      <div className="h-8 bg-white border-t border-gray-200 px-6 flex items-center justify-between text-[11px] text-slate-500 font-mono shrink-0">
        <div className="flex items-center space-x-4">
          <span>{charCount} characters</span>
          <span>{wordCount} words</span>
          <button
            onClick={() => setIsSyncDurationOpen(true)}
            className="flex items-center space-x-1.5 text-blue-600 hover:text-blue-800 font-semibold bg-blue-50/80 hover:bg-blue-100 px-2 py-0.5 rounded transition-colors"
            title="Click to sync script to an exact target timeframe"
          >
            <Clock className="w-3 h-3 text-blue-600" />
            <span>~{estimatedSeconds}s est. duration</span>
